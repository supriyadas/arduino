...
Ticker ticker,ticker1,tickerMainLoop, tickerMainLoop1;
SimpleTimer timer;
...
void onBtnPressed()
{
  ...
  delay(200);
  tick();
  displayMode = (displayMode == 1 ? 2 : (displayMode == 2 ? 3 : (displayMode == 3 ? 4 : (displayMode == 4 ? 1 : displayMode))));
  upadteDisplay();
}
...
void initSetup() {
  ...
  pinMode(CT_LED_PIN, OUTPUT);//LED ind ctrl
  digitalWrite(CT_LED_PIN,HIGH);
